...
\begin{itemize}
  ...
  \item In $A_{-3}$: Let $\alpha = a + b \omega$, then
    $p = \alpha \bar{\alpha} \iff p^2 = a^2 + b^2 - ab \iff 4p = (2a - b)^2 + 3b^2$
    ($p = \alpha \beta$ 怎辦 ??)
    ...
    If $p$ is a prime in $\Zb[\omega]$, then $p \mid (a+1-2\omega)$ or $p \mid (a - 1 - 2\omega)$,
    but $p \mid a + b \mathrm{i} \implies p \mid 2b$ (since there are half integer in $A_{-3}$),
    so $p \mid 2$, which leads to an contradiction,
...
\begin{prop} \label{prop:property-of-prime-and-primary} \hfill
  \begin{itemize}
