...
\begin{definition}
  ...
  $\mathbf{x} =(x_1, \dots, x_n)$ with $x_i \in R$, define
  \[
...
\begin{coro}
  ...
  \begin{proof}
    ...
    \[
      z \xmapsto{\pi^{-1}} (0, z) \xmapsto{d} \left((-1)^{i-1}xz, 0\right)
      \xmapsto{\rho^{-1}} (-1)^{i-1} xz.
    \]
  \end{proof}
\end{coro}
...
\begin{definition}
  Let $M \in \Mod_R$. A sequence $\set{a_1, \dots, a_m}, m \ge 0$ is said to be
  $M$-regular if
  \begin{itemize}
    \item $M/\gen{a_1, \dots, a_m}M \neq 0$.
    \item $a_{i+1}$ is not a zero divisor of $M/\gen{a_1, \dots, a_i}M$ for 
      $0 \le i \le m-1$.
  \end{itemize}
\end{definition}

\begin{theorem}
  If $\mathbf{x} = (x_1, \dots, x_n)$ is an $R$-regular sequence, then
  $K_\bullet(\mathbf{x}) \to R/\gen{x_1, \dots, x_n} \to 0$ is a free resolution of 
  $R / \gen{x_1, \dots, x_n}$.
  ...
  \begin{proof}
    ...
    \begin{itemize}
      ...
      \item $n > 1$:  
        Assume that $\mathbf{x}' = (x_1, \dots, x_{n-1})$ and 
        $K_\bullet(\mathbf{x}') \to R/\gen{x_1, \dots, x_{n-1}} \to 0$ exact, 
        ...
        For $i = 1$, we have $H_0(K_\bullet(\mathbf{x}')) \cong R/\gen{x_1, \dots, x_{n-1}}$, so
        \[
          0 \to H_1(K_\bullet(\mathbf{x})) \to R/\gen{x_1, \dots, x_{n-1}} \xto{\pm x_n} 
          R/\gen{x_1, \dots, x_{n-1}}
        \]

        But $x_n$ is not a zero divisor of $R/\gen{x_1, \dots, x_{n-1}}$, so the map
...
\begin{coro}
  Let $I = \gen{x_1, \dots, x_n} \subset R$ be an ideal with $\set{x_1, \dots, x_n}$ be $R$-regular, 
  ...
  \begin{proof}
    ...
    But 
    \[
      \Lambda^n R^n = R(e_1 \we \dotsb \we e_n) \cong R, \quad
      \Lambda^{n-1} R^n = \bigoplus\limits_{i=1}^n  R (e_1 \we \dotsb \we \hat{e_i} \we \dotsb \we e^n)
      \cong R^n
    \]
    so
    \[
      \deffunc{d_n}{R}{R^n}{r}{(x_1 r, -x_2 r, \dots, (-1)^{n-1} x_n r)}
    \]
    ...
    \[
      \deffunc{d_n \Ot \id}{R/I}{(R/I)^n}{\ob{r}}{(\ob{x_1r}, \ob{-x_2r}, \dots, \ob{(-1)^{n-1}x_n r})}
    \]
    ...
    \]
    ($R/I \neq 0$ is because $\set{x_1, \dots, x_n}$ is $R$-regular.)
...
\begin{remark}
  Let $I = \gen{x_1, \dots, x_n}$ generated by $R$-regular sequence 
  $\set{x_1, \dots, x_n}$, then
  \begin{itemize}
...
\begin{theorem}
  ...
  \[ \begin{tikzcd}[column sep=tiny]
      & & {\color{red} \exists U}
      \ar[ld, "S \ni u"', red] \ar[rd, "h", red] & \text{\footnotesize \color{red} by 2.} & \\
