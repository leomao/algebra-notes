...
\begin{theorem}[Eisenstein criterion] \mbox{} \\
  ...
  Assume that $\exists$ a prime $p$ s.t. $p \nmid a_n$ but $p \mid a_i$ for other $i \neq n$,
...
\begin{prop}
  ...
  minimal degree s.t. $m_{\alpha, K}(\alpha) = 0$ and for any other
...
\begin{proof}
  ...
  By theorem \ref{thm:field-ext-1}, $\exists$ an extension $K(\alpha_1)$ s.t. $p(\alpha_1) = 0$.
