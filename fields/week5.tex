...
\begin{prop} \hfill
  \begin{itemize}
    ...
    \item $\Gal(\Qb(\zeta_n) / \Qb) \cong \Fb_n^\times$.
      ...
      \begin{proof}
        Let $\sigma_k = (\eta_n \mapsto \eta_n^k) \in \Gal(\Qb(\zeta_n) / \Qb)$.
        The isomorphism is given by $\sigma_k \mapsto \bar{k}$.
        Clearly, it is a homomorphism since $\sigma_k \sigma_h = (\eta_n \mapsto \eta_n^{kh}) = \sigma_{kh}$.
...
\[ \Gal\big( \Qb(\eta_n) / \Qb \big) \cong
  \Gal\big( \Qb(\eta_{p_1^{n_1}}) / \Qb \big) \times
  \Gal\big( \Qb(\eta_{p_1^{n_2}}) / \Qb \big) \times \cdots \times
  \Gal\big( \Qb(\eta_{p_1^{n_k}}) / \Qb \big) \]
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    Then $G$ is a subgroup of $\Zb / (p_1 - 1) \Zb \times \dots \times \Zb / (p_k - 1) \Zb \cong \Gal(\Qb(\eta_n) / \Qb)$
    where $n = p_1 p_2 \dotsm p_k$.
  \end{proof}
\end{theorem}

\subsection{Kummer extension}
In this section, we assume that $\Char K \nmid n$ and $\eta$ is a primitive $n$th root of unity.

\begin{definition} \hfill
  \begin{itemize}
    \item $L/K$ is called a kummer extension of exponent $n$ if $\eta \in K$ and $L$ is a splitting field
