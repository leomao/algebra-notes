...
\begin{definition} \hfill
  \begin{itemize}
    \item The cylotonic field $\Qb(\zeta_n)$ is the splitting field of $x^n - 1$ over $\Qb$.
    ...
    \item $\zeta_n$ is called a primitive nth root of unity if $\zeta^n = 1$ but $\zeta^m = 1, \, \forall 0 < m < n$.
...
\begin{prop} \hfill
  \begin{itemize}
    \item $x^n - 1 = \prod_{d \mid n} \Phi_d$. Both sides have no multiple root.
    \item $\Phi_n \in \Zb[x]$. By induction on $n$. $n = 1$ is trivial.
    \item $\Phi_n$ is irreducible.
      \begin{proof}
        ...
        Assume that $g(\zeta_n^p) = 0$, since $m_{\zeta_n, \Qb} = f$, $f \mid g(x^p)$,
        say $g(x^p) = f(x) h(x)$.
      \end{proof}
    \item $\Qb(\zeta_n) / \Ab$ is Galois with $[\Qb(\zeta_n): \Qb] = \deg \Phi_n = \varphi(n)$.
